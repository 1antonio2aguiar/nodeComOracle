module.exports = function(application){
    ...
    application.get('/tiposContatosList', function(req, res){
        var query_select = ''; 
        var query_count  = '';
        var pagina = 0;
        ...
        async function run() {
            ...
            query_count = 'select count(*) from tipos_Contatos' ;
            ...
            connOracleDb.execute(query_count, function(error, result){
                ...
                query_select = 'select * from (select topn.*, ROWNUM rnum from ' +
                    '(select id,descricao from tipos_Contatos order by id ' +
                ...
                connOracleDb.execute(query_select, {fim: fim, inicio: inicio}, function(error, result) {
                    ...
                    res.render("tiposContatos/tiposContatosList", {tiposContatos : result ? result.rows : [],
    ...
    application.post('/tiposContatosSave', function(req, res){
        var id        = req.body.id;
        var descricao = req.body.descricao;

        if(id == 0){
            var query_insert = 'insert into tipos_Contatos (descricao) values (:descricao) ';
            ...
            var query_update = 'update tipos_Contatos set descricao = :descricao where id = :id ';
    ...
    application.delete('/tiposContatosDelete', function(req, res){
    
        var id = req.body.id;

        var query_delete = 'delete from tipos_Contatos where id = :id';
...
function pagination(paginator) {
    ...
    for (var i = minPage; i <= maxPage; i ++) {
        html += "<li class='page-item " + (i == paginator.page ? 'active' : '') + "'>";
            html += "<a class='btn btn-success btn-xs' href='/tiposContatosList?pagina=" + i + 
            "'>" + i + "</a>";
        html += "</li>";
    }
    return html;
