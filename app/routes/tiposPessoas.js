module.exports = function(application){
    ...
    application.get('/tiposPessoasList', function(req, res){
        var query_select = ''; 
        var query_count  = '';
        var pagina = 0;
        ...
        async function run() {
            ...
            query_count = 'select count(*) from tipos_pessoas' ;
            ...
            connOracleDb.execute(query_count, function(error, result){
                ...
                query_select = 'select * from (select topn.*, ROWNUM rnum from ' +
                    '(select id,descricao from tipos_pessoas order by id ' +
                ...
                connOracleDb.execute(query_select, {fim: fim, inicio: inicio}, function(error, result) {
                    ...
                    res.render("tiposPessoas/tiposPessoasList", {tiposPessoas : result ? result.rows : [],
    ...
    application.post('/tiposPessoasSave', function(req, res){
        var id        = req.body.id;
        var descricao = req.body.descricao;

        if(id == 0){
            var query_insert = 'insert into tipos_pessoas (descricao) values (:descricao) ';
            ...
            var query_update = 'update tipos_pessoas set descricao = :descricao where id = :id ';
    ...
    application.delete('/tiposPessoasDelete', function(req, res){
    
        var id = req.body.id;

        var query_delete = 'delete from tipos_pessoas where id = :id';
...
function pagination(paginator) {
    ...
    for (var i = minPage; i <= maxPage; i ++) {
        html += "<li class='page-item " + (i == paginator.page ? 'active' : '') + "'>";
            html += "<a class='btn btn-success btn-xs' href='/tiposPessoasList?pagina=" + i + 
            "'>" + i + "</a>";
        html += "</li>";
    }
    return html;
